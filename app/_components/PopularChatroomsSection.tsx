    ...
    <section className="py-12">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap items-end justify-between gap-4 mb-12">
          <div>
            <p className="text-[#BB0015] font-bold tracking-wider">POPULAR ROOMS</p>
            <h2 className="text-4xl font-bold mt-2 text-slate-900">熱門聊天室</h2>
          </div>
          <Link
            href="/user/chatroom"
          ...
          {loading || rooms.length === 0
            ? // Skeleton 載入骨架屏
              Array.from({ length: 4 }).map((_, index) => (
                <div key={index} className="overflow-hidden bg-white shadow animate-pulse rounded-lg">
              ...
              rooms.map((room) => (
                ...
                >
                  <div>
                    ...
                    {/* 卡片內容 */}
                    <div className="p-5 space-y-3">
                      <p className="text-sm text-slate-500 font-medium">聊天室 #{room.id}</p>
                  ...
                  <div className="px-5 pb-5 pt-2 flex items-center justify-between border-t border-slate-100 text-sm text-slate-600">
                    <div className="flex items-center gap-1">
                      <span className="text-[#BB0015] font-black text-lg">❤️ {room.favoriteCount || 0}</span>
                      <span className="text-xs text-slate-400 ml-0.5">人收藏</span>
                    </div>
